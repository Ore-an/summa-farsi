...
mkdir -p $decode_dir/score_$LMWT
...
lattice-to-ctm-conf $frame_shift_opt --decode-mbr=$decode_mbr ark:- $decode_dir/score_$LMWT/penalty_$wip/$name.ctm.overlapping || exit 1;

ctm=$decode_dir/score_$LMWT/penalty_$wip/$name.ctm.overlapping
...
if $resolve_overlaps; then
  utils/ctm/resolve_ctm_overlaps.py $data_dir/segments \
    $decode_dir/score_$LMWT/penalty_$wip/$name.ctm.overlapping \
    $decode_dir/score_$LMWT/penalty_$wip/$name.ctm.merged || exit 1;
  ctm=$decode_dir/score_$LMWT/penalty_$wip/$name.ctm.merged
fi

cat $ctm | utils/int2sym.pl -f 5 $lang/words.txt | \
utils/convert_ctm.pl $data_dir/segments $data_dir/reco2file_and_channel | \
sort -k1,1 -k2,2 -k3,3nb > $decode_dir/score_$LMWT/penalty_$wip/$name.ctm || exit 1;
# Remove some stuff we don't want to score, from the ctm.
$filter_ctm_command  $decode_dir/score_${LMWT}/penalty_$wip/$name.ctm  $decode_dir/score_${LMWT}/penalty_$wip/$name.ctm.temp

awk '$4 < 0.75 + 0.2*length($5)' < $decode_dir/score_${LMWT}/penalty_$wip/$name.ctm.temp \
  | perl -ane '@A = split; $word = $A[4]; if ($word =~ s/\._//g) { $word =~ s/\.$//; $word =~ s/.s/s/; } $A[4] = $word; print join("\t", @A), "\n"; ' \
  > $decode_dir/score_${LMWT}/penalty_$wip/$name.ctm.filt || exit 1;
rm $decode_dir/score_${LMWT}/penalty_$wip/$name.ctm.temp
...
if [ ! -z $stm ]; then
  ...
  $hubscr -p $hubdir -V -l english -h hub5 -g $glm -r $stm $decode_dir/score_$LMWT/penalty_$wip/$name.ctm.filt || exit 1;
fi
